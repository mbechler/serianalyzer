...
public class SerianalyzerMethodVisitor extends MethodVisitor {
    ...
    public SerianalyzerMethodVisitor ( SerianalyzerClassVisitorBase parent, MethodReference ref ) {
        ...
        this.log = Logger.getLogger(eu.agno3.tools.serianalyzer.SerianalyzerMethodVisitor.class.getName() + "." + ref.getTypeNameString()); //$NON-NLS-1$
        this.parent = parent;
        this.ref = ref;
    ...
    public void visitFieldInsn ( int opcode, String owner, String name, String desc ) {
        ...
        // static fields starting with class$ seem to be generated by compilers to cache referenced classes
        if ( opcode == Opcodes.GETSTATIC && !name.startsWith("class$") ) { //$NON-NLS-1$
        ...
        if ( tainted ) {
            if ( this.log.isDebugEnabled() ) {
                this.log.debug(String.format(
                    "Tainted invoke%s method %s::%s (%s,%d,%s)", opcode == Opcodes.INVOKESTATIC ? " static" : "", clName, name, desc, opcode, itf)); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
                this.log.debug("  Arguments " + args); //$NON-NLS-1$
                if ( opcode != Opcodes.INVOKESTATIC ) {
            ...
            if ( this.log.isDebugEnabled() ) {
                this.log.debug(String.format(
                    "Untainted invoke%s method %s::%s (%s,%d,%s)", opcode == Opcodes.INVOKESTATIC ? " static" : "", clName, name, desc, opcode, itf)); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
                this.log.debug("  Arguments " + args); //$NON-NLS-1$
                if ( opcode != Opcodes.INVOKESTATIC ) {
                    this.log.debug("  Target " + tgt); //$NON-NLS-1$
                }
